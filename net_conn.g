...
//Sriraman Damodaran dsriraman@gmail.com
...
//Used to call FS network - though it bears similarities with the SP network call kept separate mostly because of gap junctions
...
function netconn(net,net2,network,network2,fact,loops,syn_weight1,chem_gap,ind_gap, gap_prob, ind_prob, gap_res)
	...
	for(ctrpre = 0; ctrpre < {getglobal numCells_{net}}; ctrpre = {ctrpre + 1})  //loops through presynaptic cells
		...
		for(ctrpost = 0; ctrpost < {getglobal numCells_{net2}}; ctrpost= {ctrpost +1})  //loops through postsynaptic cells
			if ({ctrpre}!={ctrpost}) // connect only if pre and post synaptic cells are different                                                                                                 
...
/*
				if (({getfield /SPnetwork/SPcell[{ctrpre}]/soma D1} == 1) && ({getfield /SPnetwork/SPcell[{ctrpost}]/soma D1} == 0))
					prob=0
				end
*/
		...
		*/
       		...
       		//echo {{getfield pre_spk X_A->table[{ctrpost}]}/227} >> SP.log 
       		//echo {{getfield pre_spk X_B->table[{ctrpost}]}/227} >> FS.log 
       		//echo {{getfield pre_spk Y_A->table[{ctrpost}]}/227} >> ext.log 
  	end     
end 
