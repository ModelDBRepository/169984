 		...
 		c_ctr = 0
        	...
        	while ({c_ctr} < {int_GABA})  
				...
				if ({DA}=="D1")
				 ...
				 prob=0.43 //to double have to increase to this value //FS- 62.5%
				end
...
/*
				...
				elif (({getfield /SPnetwork/SPcell[{ctrpre}]/soma D1} == 1) && ({getfield /SPnetwork/SPcell[{ctrpost}]/soma D1} == 0))
					prob=0
*/				
				end
		...
		*/
		...
		if ({DA}=="D1")
			...
			echo {{getfield pre_spk X_A->table[{ctrpost}]}/145} >> SP_D2.log 
       			echo {{getfield pre_spk X_B->table[{ctrpost}]}/145} >> FS_D2.log 
